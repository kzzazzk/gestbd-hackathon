...
# 2. Obtener descripciones desde Wikidata
def get_wikidata_description(term):
    """
    Fetches the description of a term from the Wikidata API.
    """
    url = "https://www.wikidata.org/w/api.php"
    params = {
        "action": "wbsearchentities",
        "format": "json",
        "language": "en",  # Using English for consistency
        "search": term
    }
    # Set a user-agent header as required by Wikidata's API policy
    headers = {
        "User-Agent": "MyCoolClusteringApp/1.0 (test@example.com)"
    }
    
    try:
        r = requests.get(url, params=params, headers=headers)
        r.raise_for_status()  # This will raise an error for bad responses (4xx, 5xx)
        
        data = r.json()
        
        # Check if the 'search' key exists and has results
        if "search" in data and len(data["search"]) > 0:
            # Take the description from the *first* result
            first_result = data["search"][0]
            description = first_result.get("description", "")
            print(f"  [OK] Found description for: {term}")
            return description
        else:
            print(f"  [!!] No description found for: {term}")
            return ""
    
    except requests.exceptions.RequestException as e:
        print(f"  [ERROR] API request failed for {term}: {e}")
        return ""


print("Fetching descriptions from Wikidata...")
df["descripcion"] = df["nombre"].apply(get_wikidata_description)
...
n_clusters = 50  # You can change this number
